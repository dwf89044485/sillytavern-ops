...
> - **v2.1（2026-08-13）：⚠️ 标注可靠性缺陷。见下方「本文件的效力边界」。**

---

## ⚠️ 本文件的效力边界（v2.1 补，必读）

**v1/v2 的碰撞是在"零教程原文阅读"的条件下做的。** 分析者只读了三条线的**衍生决策文档**，未读：

- `性格调色盘/` 11 篇教程原文 + `10.大总结教程/` 7 章
- `性格调色盘/_AI_READ_THIS_FIRST.md`（AGENTS.md 点名"读教程前必读"的翻译层定义）
- `性格调色盘/工作流/` 24 问 / 80 问 / 200 问 / AI 循环问答工作流
- **A 线 `candidates/` 的一阶提取物**（240KB：框架 18 / 原则 110 / 案例 20 / 反例 53 / 术语 34）

**后果分三级：**

| 级别 | 受影响的部分 | 说明 |
|---|---|---|
| 🔴 **已证实有错** | §2.1 净增量表 B-3、B-4 | 见下「已发现的错误」 |
| 🟠 **高风险未验证** | 全部 ⚪ 伪冲突判定（F1–F11）、🔴 D3/D4/F8 | 这些裁定依赖"某条线对教程的引用是否忠实"，而分析者无法核对任何一条教程引用 |
| 🟡 **中风险未验证** | 全部"某线完全没有 X"式断言 | 只核对了三条线的**收敛版**文档，未核对各自的原始提取物 |

**🔴 已发现的错误（v2.1 修正）**

抽查 A 线 `candidates/counter-examples.md` 后发现两条断言是错的：

- **原 B-3**「矛盾检测的放大器规则是 B 的净增量，A 只有例子没有判据」→ **错**。A 线 `ce43`（"在性格标签前加'极度''极致'等放大器"）不仅有，而且比 B 更完整：带 `source_quote`（大总结 02 调色盘篇 §三原文）、`mechanism`（"程度副词是 LLM 语义空间中的强度放大器，改变的不只是标签强度，而是整个行为模式的允许范围"）、`warning_signs` 三条。
- **原 B-4**「结论检测是 B 的净增量，A 只写了'无解释词'」→ **错**。A 线 `ce44`（"背景写'这是她性格转变的起点'等因果总结"）有完整的 failure_mode + mechanism + warning_signs。
- **原 B-5**「扁平检测 A/C 完全没有」→ **经核对属实**，A 线 candidates 无对应条目。

**★ 这个错误暴露的问题比错误本身重要：**

这两条**不是 B 的净增量，是 A 线自己的收敛损耗**——A 在阶段 1 就提取到了，但从 `candidates/` 收敛进 `CHAR_BUILD_GUIDE_DESIGN.md` 的过程中丢了。两者的处理动作完全不同：

- 若是「B 的净增量」→ 动作是"把 B 嫁接进 A"
- 若是「A 的收敛损耗」→ 动作是"**A 线必须回头审计 candidates/ → design 的完整丢失清单**"，而 B 只是独立重新发现了 A 已经挖到又弄丢的东西

240KB 的一阶提取物里**很可能还有更多这样的丢失项**。这是一条本文件尚未展开的独立工作线。

**在补齐教程原文与 candidates/ 阅读之前，本文件的 🟢 共识与 🔴 冲突清单可以用作讨论议程，但不能用作裁决依据。**
...
| ~~**B-3**~~ | ~~矛盾检测的触发器是放大器不是标签本身~~ | 🔴 **v2.1 撤回**。A 线 `candidates/counter-examples.md` 的 `ce43` 已有此规则且更完整（含教程原文引用 + 机制解释 + 三条警示信号）。**这是 A 的收敛损耗，不是 B 的净增量**。B 的增量仅剩"把它接进矛盾诊断判定树"这一步 |
| ~~**B-4**~~ | ~~结论检测~~ | 🔴 **v2.1 撤回**。A 线 `ce44`（背景写因果总结）已有完整 failure_mode + mechanism。**同属收敛损耗**。B 的增量仅剩"摄影机测试 / 瞬间动词 vs 模式动词 / 结论词清单"这套**判定工具**，不是"发现了这个问题" |
...
### 7.4 ★ v2.1 新增待办：补齐源头层 + 收敛损耗审计

**必须在任何裁决之前完成**（顺序即优先级）：

- [ ] **① 读 `性格调色盘/_AI_READ_THIS_FIRST.md`**（1942 字节）——AGENTS.md 点名"读教程前必读"的翻译层定义，本次跳过了
- [ ] **② 读 A 线 `candidates/` 全部 5 份**（240KB）——**性价比最高**：它是一阶提取物，每条带 `source_chapter` + `source_quote`，等于"带出处的教程内容"，且直接修复 §2.1/§2.2 净增量表
- [ ] **③ A 线收敛损耗审计**——逐条比对 `candidates/`（18 框架 / 110 原则 / 20 案例 / 53 反例 / 34 术语）与 `CHAR_BUILD_GUIDE_DESIGN.md`，列出"提取到了但没进设计文档"的完整清单。ce43/ce44 是抽查发现的两条，**总量未知**
- [ ] **④ 读教程原文**——重点是三条线互相引用打架的段落：
  - 大总结 02 调色盘篇（B 的诊断锚点 / A 的 G5 / ce43 都引它）
  - 教程 2 号 + 大总结 03 三面性篇（🔴 D4 构建顺序：A 说"先有调色盘再有多面性"，C 说"防御机制决定面功能"）
  - 教程 6 号 + 大总结 07 意象篇（🔴 D3 意象位置：C 说"教程只说克制字少，没要求每个角色都有"，A 说"依赖链最后"）
  - 教程 7 号 `:220`（C-4 的"最低限度要知道"清单，C 给了精确行号，未核）
  - 教程 11 号（A 的快速路径 G3 五步全部依据此篇）
- [ ] **⑤ 读 `性格调色盘/工作流/`**——A 的 question-bank / B 的 6 维度清单 / C 的十组目标层三者都源自这里，粒度关系待定（⬜ G8）
- [ ] **⑥ 复核全部 ⚪ 伪冲突判定**——F1–F11 的裁定全部依赖未经核对的教程引用
